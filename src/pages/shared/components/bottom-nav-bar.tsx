...
const navItems = [
  ...
  { name: 'Work Updates', href: '/work-update', icon: Calendar },
]
          ...
          {navItems.map((item) => {
            ...
            return (
              <li key={item.name} className="relative w-full">
                <Link
                  to={item.href}
                  className={`flex flex-col items-center justify-center h-full w-full text-xs font-medium transition-all duration-300 ${isActive ? 'text-yellow-400' : 'text-gray-400 hover:text-yellow-200'
                    }`}
                >
